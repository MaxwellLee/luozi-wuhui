...
(function (g) {
  ...
  function applyMove(pos, i, log) {
    ...
    var color = pos.toMove, opp = 3 - color;
    pos.board[i] = color;
    var nb = neighbors(pos, i);
    for (var m = 0; m < nb.length; m++) {
      var n = nb[m];
    ...
    if (own.liberties.length === 0) {      // 禁着（自杀）
      pos.board[i] = 0;
      for (var u1 = 0; u1 < rec.captured.length; u1++) pos.board[rec.captured[u1]] = opp;
      pos.caps[color - 1] -= rec.captured.length;
      return false;
    }
    pos.toMove = opp; pos.passes = 0; pos.moves++;
    pos.lastMove = i; pos.captured = rec.captured.length;
    var newHash = computeHash(pos);
    if (newHash === pos.prevHash) {        // 劫（同形禁手）
      pos.board[i] = 0;
      for (var u2 = 0; u2 < rec.captured.length; u2++) pos.board[rec.captured[u2]] = opp;
      pos.caps[color - 1] -= rec.captured.length;
      pos.toMove = rec.toMove; pos.passes = rec.passes; pos.moves = rec.moves;
      pos.lastMove = rec.lastMove; pos.captured = rec.capturedCnt;
      return false;
    }
    pos.hash = newHash; pos.prevHash = rec.hash;
    if (log) log.push(rec);
    return true;
  }
