...
(function (g) {
  ...
  function init() {
    els = {
      ...
      badge: $('mode-badge'), difficulty: $('game-difficulty'),
  ...
  function newGameFlow() {
    ...
    var ngModal = g.App.modal({
      ...
      body:
        ...
        '<option value="hard"' + (s.difficulty === 'hard' ? ' selected' : '') + '>挑战</option></select></div>' +
  ...
  function start(opts) {
    ...
    state = {
      ...
      startedAt: Date.now(), cancelAI: false, savedId: null
    };
  ...
  function updateUI() {
    ...
    els.undo.style.display = state.mode === 'formal' ? 'none' : '';
    els.hint.disabled = !s.hints || state.aiThinking || state.over || state.scoring || state.pos.toMove !== state.playerColor;
  ...
  g.GamePage = {
    init: init, newGameFlow: newGameFlow, resume: resume,
