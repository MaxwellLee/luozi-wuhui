...
(function (g) {
  ...
  var BUDGET = { medium: { 9: 500, 13: 350, 19: 200 }, hard: { 9: 1200, 13: 800, 19: 350 } };
  var HINT_BUDGET = 260;

  function ctxOf(pos) { return { color: pos.toMove, opp: 3 - pos.toMove, size: pos.size, moves: pos.moves }; }
  ...
  function candidates(pos) {
    var size = pos.size;
    var list = [], added = new Set();
    function add(i) { if (pos.board[i] === 0 && !added.has(i)) { added.add(i); list.push(i); } }
    var hasStone = false;
  ...
  function playoutMove(p, rng, log) {
    ...
    var ctx = ctxOf(p);
    // 洗牌并只对前 16 个候选做启发式评分（控制速度）
    for (var f0 = 0; f0 < cands.length; f0++) {
      var j0 = (rng() * (cands.length - f0)) | 0;
      var t0 = cands[f0]; cands[f0] = cands[f0 + j0]; cands[f0 + j0] = t0;
    }
    var top = [];
    var evalN = Math.min(8, cands.length);
    for (var k = 0; k < evalN; k++) top.push({ i: cands[k], s: heuristic(p, cands[k], ctx).score });
  ...
  function think(pos, difficulty, opts) {
    ...
    return new Promise(function (resolve) {
      ...
      uctSearch(pos, budget, rng, opts.onProgress).then(function (res) {
        ...
        if (heBest && res.rate < 0.24 && heBest.reasons.length === 0 && pos.moves > 24) resolve(-1);
